...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/08/24 13:54:44 by cogata            #+#    #+#             */
/*   Updated: 2023/08/24 13:54:45 by cogata           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || BUFFER_SIZE <= 0)
		return (NULL);
	if (next_line[fd] == 0)
		next_line[fd] = ft_strdup("");
	buffer = ft_calloc((BUFFER_SIZE + 1), 1);
	if (buffer == NULL)
		return (NULL);
	next_line[fd] = ft_read_file(buffer, &next_line[fd][0], fd);
	if (next_line[fd] == NULL)
		return (NULL);
	line = ft_get_line(&next_line[fd][0]);
	temp = next_line[fd];
	next_line[fd] = ft_get_next_line(temp);
	if (next_line[fd] != NULL)
		free(temp);
